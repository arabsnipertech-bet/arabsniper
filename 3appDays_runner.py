...
def validate_day1_pair(verbose=True):
    ...
    if not day1_dates:
        if verbose:
            print("⚠️ Data day1 non ancora coerente. Attesa: "
                  f"{expected} | Trovata: N/D", flush=True)
        return False

    if day1_dates != {expected}:
        if verbose:
            print(f"⚠️ Data day1 non ancora coerente. Attesa: {expected} | Trovata: {sorted(day1_dates)}", flush=True)
        return False
...
def run_mid_day1():
    ...
    # Nel mid-day non blocchiamo tutto se GitHub non ha ancora propagato day1 in modo perfetto.
    synced = sync_remote_outputs_to_local()

    if not synced:
        print("⚠️ Sync non perfetta, ma continuo comunque (modalità mid-day).", flush=True)
